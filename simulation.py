...
def simulatio():
   ...
   for i in range(df.shape[0]):
     res_team =results[results['Team']==df['Team'][i]]
     df['Wins'][i] = np.sum(res_team['result']==3)
     df['Draws'][i] = np.sum(res_team['result']==1)
     df['Losses'][i] = np.sum(res_team['result']==0)
     df['Goals'][i] = np.sum(res_team['Goals'])
     df['Against'][i] = np.sum(res_team['Against'])
     df['Diff'][i] = df['Goals'][i] - df['Against'][i]
     df['Points'][i] = np.sum(res_team['result'])
   df['Ranking'] = range(32)
   df = df.sort_values(['Points','Diff','Goals'],ascending=False)
   for group in groups: 
      df_group = df[df['Group']==group]
